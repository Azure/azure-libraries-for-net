// <auto-generated>
// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License. See License.txt in the project root for
// license information.
//
// Code generated by Microsoft (R) AutoRest Code Generator.
// </auto-generated>

namespace Microsoft.Azure.Management.Cdn.Fluent.Models
{
    using Newtonsoft.Json;
    using System.Linq;

    /// <summary>
    /// Output of the validate probe API.
    /// </summary>
    public partial class ValidateProbeOutputInner
    {
        /// <summary>
        /// Initializes a new instance of the ValidateProbeOutputInner class.
        /// </summary>
        public ValidateProbeOutputInner()
        {
            CustomInit();
        }

        /// <summary>
        /// Initializes a new instance of the ValidateProbeOutputInner class.
        /// </summary>
        /// <param name="isValid">Indicates whether the probe URL is accepted
        /// or not.</param>
        /// <param name="errorCode">Specifies the error code when the probe url
        /// is not accepted.</param>
        /// <param name="message">The detailed error message describing why the
        /// probe URL is not accepted.</param>
        public ValidateProbeOutputInner(bool? isValid = default(bool?), string errorCode = default(string), string message = default(string))
        {
            IsValid = isValid;
            ErrorCode = errorCode;
            Message = message;
            CustomInit();
        }

        /// <summary>
        /// An initialization method that performs custom operations like setting defaults
        /// </summary>
        partial void CustomInit();

        /// <summary>
        /// Gets indicates whether the probe URL is accepted or not.
        /// </summary>
        [JsonProperty(PropertyName = "isValid")]
        public bool? IsValid { get; private set; }

        /// <summary>
        /// Gets specifies the error code when the probe url is not accepted.
        /// </summary>
        [JsonProperty(PropertyName = "errorCode")]
        public string ErrorCode { get; private set; }

        /// <summary>
        /// Gets the detailed error message describing why the probe URL is not
        /// accepted.
        /// </summary>
        [JsonProperty(PropertyName = "message")]
        public string Message { get; private set; }

    }
}
